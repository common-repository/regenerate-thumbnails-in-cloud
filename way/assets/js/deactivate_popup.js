...
function wdReady(prefix){
	...
	jQuery(document).on("change", "[name=" + prefix + "_reasons]", function(){	
		...
		if(jQuery(this).val() == "reason_plugin_is_hard_to_use_technical_problems"){
			...
			additionalInfo = '<div class="wd-additional-active"><div><h2>Your feedback will help to improve</h2></div><br>' +
							...
							// '<br><div><button class="button button-primary wd-' + prefix + '-deactivate" data-val="' + btnVal + '">Submit support ticket</button></div>'
							'</div>';
			...
			additionalInfo = '<div class="wd-additional-active">' + 
								'<div><strong>Try Pro Plan from 0.5$.Consider upgrading.  </strong></div>' +
								'<div><a href=\'https://way2enjoy.com/regenerate-thumbnails?pluginemail='+ window[prefix + "WDDeactivateVars"].email +'\' target="_blank">Upgrade with Paypal.</a>. Get 100000 Credit free if you found plugin which gives better quality</div>';
			...
			additionalInfo = '<div class="wd-additional-active">' + 
								'<div><strong>We have a special offer for you.</strong><a href=\'https://way2enjoy.com/regenerate-thumbnails?pluginemail='+ window[prefix + "WDDeactivateVars"].email +'\' target="_blank">Upgrade with Paypal.</a></div>' +
...
setTimeout(function() {
$('#-reason_plugin_is_hard_to_use_technical_problems').trigger('change');
 })
}, 3000);
